...
- **First-script gaps:** tabs leaving the New Tab Page, measured by phase 03 at 10 misses in 10 loads and by phase 06's Audit at 20 misses in 20 loads, because Chrome refuses the attach until the next document has committed; and a tab created with its url already set, measured by phase 03 at 10 misses in 10 tabs against a server on the same machine, because `chrome.tabs.onCreated` reaches the service worker about 12 ms after the document request is already in flight. That second gap is about 25 ms wide for a document that is fetched: the same page served with 25 ms of latency misses none. A document the browser already holds closes that window entirely, measured by phase 06 at 10 misses in 10 tabs opened straight onto a page served with `Cache-Control: max-age=3600`, and at 10 misses in 10 again with 25 ms of server latency, because neither set of loads reached the server at all (0 requests for 10 loads). Both tabs are Covered a few milliseconds later. Popups, prerendered pages and shared workers were the other candidates here, and phase 06 measured none of them missing: 0 misses in 5 same-site popups and 0 in 5 cross-site popups, each of the ten also reading the Selection's point from a position call started in that same first script, the shared worker's first line reading the Override, and no prerender happening under the harness at all.
- **The bar:** `resize` events and a smaller `innerHeight` on open tabs in a headed browser while Spoofer is attached. Measured by phase 06 headed with no fixed viewport at an `innerHeight` of 714 before the attach and 658 six seconds after it, a delta of 56 px over 8 `resize` events. A headless browser has no bar and shows none of this.
...
- **Pause latency:** a child target is paused at start until Spoofer has sent it the zone. Measured by phase 06 over five runs at a median 8.5 ms to a cross-origin iframe's `load` against the Baseline's 6.8 ms, and a median 3.0 ms to a dedicated worker's first message against 2.2 ms.
