...
3. **e2e** `with no Selection a page observes the Baseline zone`: `UTC` and offset `0`, because nothing attaches without a Selection.
...
Rules for the green code: adapters are the only modules that touch `chrome`; the debugger adapter sends only the four methods the brief allows; a failed attach on a restricted target is recorded, never retried in a loop.
