...
Sojourn changes the time zone and the geolocation position a page observes and nothing else, and its own footprint is only what ADR-0001's mechanism needs. Each item below will look like a cheap consistency win later ("we already set the zone, why not the language"), and each is refused on purpose: every added surface is a new place for a Trace, a new line on the install prompt, or a new way to expose the user.

- No network requests from any extension context.
- No change to `navigator.language`, `Accept-Language`, the user agent, screen, canvas, fonts, WebRTC, IP, DNS, or any other fingerprint surface. The README pairs Sojourn with a VPN instead.
