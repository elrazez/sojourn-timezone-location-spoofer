...
test('a page opened after selecting Tokyo observes Asia/Tokyo', async ({ context, origins, sojourn }) => {
...
test('with no Selection a page observes the Baseline zone', async ({ context, origins, sojourn }) => {
  const page = await context.newPage();
  await page.goto(origins.localhost);

  // A discriminating transition rather than a wait: Sojourn proves it can move this page, and only
...
test('changing the City from Tokyo to Los Angeles reaches an open tab with no reload', async ({
  ...
  await sojourn.select('los-angeles');
...
test('a covered page prints Japan Standard Time in Date.toString', async ({ context, origins, sojourn }) => {
  await sojourn.select('tokyo');
  const page = await context.newPage();
  await page.goto(origins.localhost);
  await expect.poll(() => readZone(page)).toBe(TOKYO_ZONE);
